...
export function Products({ className, query }) {
    ...
    const [ allProducts, setAllProducts ] = useState((null as any));
    ...
    useEffect(() => {
        ...
        if (queryValue && nextPage == 0) {
            setProductsLength(0);
            const limit = paginationFromQuery?.limit ? paginationFromQuery?.limit : 0;
            const hayOffset = paginationFromQuery?.offset;
            const offset = hayOffset ? hayOffset : 0; 

            getProductsByQuery(queryValue, limit, offset).then((dataFromAPI) => {
        ...
        if (queryValue && nextPage != 0) {
            ...
            getProductsByQuery(queryValue, limit, offset).then((dataFromAPI) => {
                setProductsByQuery(dataFromAPI?.results);
                setpaginationFromQuery(dataFromAPI?.pagination);
                setProductsLength(productsLength + dataFromAPI?.results.length);
            });
        }
    ...
    return <div className={className} id="top">

        { productsByQuery && productsLength ==  paginationFromQuery?.total ?
            <div style={{ display: "flex", padding: 10, flexDirection: "column" }}>

                <SubtitleStyle> Resultados: { productsLength } de { paginationFromQuery?.total } </SubtitleStyle>
        ...
        : productsByQuery && productsLength <= paginationFromQuery?.total ?
            ...
            <div style={{ display: "flex", padding: 10, flexDirection: "column" }}>
                ...
                <a onClick={nextProductsPage} style={{ color: "violet", fontSize: 30, alignSelf: "center", textDecoration: "underline", margin: 20 }}> Ver más ↪ </a>
            </div>

        :

        <div className="container">
            <SubtitleStyle> Buscá el producto que tanto querés! </SubtitleStyle>

            <div className="product-container">
                {allProducts?.map((product) => {
                    return <ProductCard id={product.objectID} description={product.Description} key={product.objectID} src={product.Images[0].url} alt="product image" productName={product.Name} price={product["unit_price"]} />
                })}
            </div>
